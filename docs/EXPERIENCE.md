...
The Experience crate also owns the versioned `RunResultReceipt` wire contract shared by the daemon and downstream evaluators. Its parser accepts only schema 1 receipts under the fixed `runtime-plane` actor and IDs derived from the run, with canonical Genome, World, source-revision, and artifact identities, bounded latency and trace cardinality, an exact deterministic zero cost, and a supported runtime-owned completion reason. This moves run-result provenance out of control-plane-private JSON so Arena trials can resolve evidence from verified events without trusting caller-populated metrics.
