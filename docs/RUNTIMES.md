...
The non-billable process supervisor proves the shared lifecycle mechanics with real local helpers. It starts each child in its own process group and allowlisted environment, streams the prompt through stdin, drains stdout and stderr concurrently into one combined byte ceiling, enforces the wall deadline independently of callers, kills descendants on interrupt, and exposes only bounded artifact paths and exit state. Credential mediation, verified provider-specific resume, and a hard Codex cost limiter remain prerequisites before hosted builders can become active adapters.
