...
- The runtime trust checkpoint requires every supervised external launch to traverse the detected OS isolation policy, records adapter-owned completion reasons and elapsed time, reserves terminal evidence capacity, and retains failed containment and unpersisted observations for retry. The current local gate holds all twenty-one critical modules above 95% and defines 163 deliberate mutations.
...
- The measurement-only Arena now accepts task plans made solely of daemon-signed runtime-result event IDs, resolves their Genome/World/source/output provenance from verified history, enforces exact World-committed task input plus seed/environment/budget pairing, validates verifier/manifests/evaluator semantics before publication, exposes a visible-only summary, and records deterministic idempotent evaluator receipts. It is not promotion-grade until mutually isolated candidate/evaluator execution and statistical selection are proven.
